...
(function(window, document) {
   ...
   var SoundSample = Juicy.SoundSample = function(src, loop, volume) {
   ...
   var SoundMultiSample = function(src, count, loop, volume) {
      ...
      for (var i = 0; i < count; i ++) {
         this.elements.push(new SoundSample(src, loop, volume));
      }
   ...
   Game.getContext = function() { return Game_context; };
   ...
   Component.create('Box', {
      ...
      render: function(context, x, y, w, h) {
         x -= this.padding[0];
         w += this.padding[0] + this.padding[2];
         y -= this.padding[1];
         h += this.padding[1] + this.padding[3];
         ...
         if (this.strokeStyle) {
            ...
            var lineWidth = context.lineWidth = this.lineWidth || 2;
   ...
   Component.create('Text', {
      ...
      set: function(config) {
         ...
         // Set attributes
         info.font = typeof(config.font) != 'undefined' ? config.font : info.font;
         info.text = typeof(config.text) != 'undefined' ? config.text : info.text;
         info.fillStyle = typeof(config.fillStyle) != 'undefined' ? config.fillStyle : info.fillStyle;
